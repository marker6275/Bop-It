...
#define ANALOG_TWIST_IN NRF_SAADC_INPUT_AIN1

#define ADC_TWIST_CHANNEL 1
...
static void adc_init(void) {
  ...
  nrf_saadc_channel_config_t light_channel_config = NRFX_SAADC_DEFAULT_CHANNEL_CONFIG_SE(ANALOG_TWIST_IN);
  error_code = nrfx_saadc_channel_init(ADC_TWIST_CHANNEL, &light_channel_config);
}
...
float twist(void) {
  ...
  bool twisted = fabsf(start - adc_sample_blocking(1)) > 0.5;

  while (!twisted) {
    twisted = fabsf(start - adc_sample_blocking(1)) > 0.5;
  }
